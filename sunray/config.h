...
#define NEARWAYPOINTSPEED           0.25  // (m/s) defines the speed of mower when reaching/leaving a waypoint
#define NEARWAYPOINTDISTANCE        0.45  // (m) defines the distance of mower if approaching and leaving a waypoint and thus triggers NEARWAYPOINTSPEED of mower
...
#define OBSTACLEAVOIDANCEWAY        0.45  // (m) distance in meters the mover will drive reverse in case of obstacle triggers
...
#define MOW_RPM_SLOW                3300  // (3400)(rpm) mow motor rpm when MOW_RPMtr_SLOW (%) of MOW_RPM_NORMAL (rpm) is met. Should be higher or the same as MOW_RPM_NORMAL
#define MOW_RPM_RETRY               3600  // (3600)(rpm) mow motor rpm when MOW_RPMtr_RETRY (%) of MOW_RPM_NORMAL (rpm) is met. Should be higher or the same as MOW_RPM_SLOW4 (is only used by ESCAPE_LAWN)
...
//#define ESCAPE_LAWN_MOWMOTORPOWER   true //not yet implemented... will get rid of RPM triggers and use power of mowmotor instead of rpm readings to trigger this function (testing)
#define MOW_RPMtr_STALL             60    // (70)(%) if RPM of mowmotor stalls under % of MOW_RPM_NORMAL mower will back up with ESCAPELAWNDISTANCE and ESCAPELAWNSPEED and try again
...
//try to fix 8308 driver with moving, only works reliable with "DOCK_RETRY_TOUCH = true"! (keep FALSE if you have no issues or no DRV8308)
...
#define SONAR_LEFT_OBSTACLE_CM   40     // slow down or stop mowing operation below this distance (cm) 
#define SONAR_CENTER_OBSTACLE_CM 40      // slow down or stop mowing operation below this distance (cm) 
#define SONAR_RIGHT_OBSTACLE_CM  40      // slow down or stop mowing operation below this distance (cm) 
